...
if page == "GP Summary":
    ...
    kpi = run_query(f"""
        WITH completed_jobs AS (
            SELECT j.id AS job_id, j.completed_on, j.business_unit_id,
            ...
            WHERE j._fivetran_deleted = FALSE AND j.job_status = 'Completed'
              AND j.completed_on BETWEEN '{start_date}' AND '{end_date}' {dept_clause}
        ),
        job_totals AS (
            SELECT SUM(gross_profit) AS gp_no_labor, SUM(revenue) AS total_revenue, COUNT(*) AS total_jobs
            FROM completed_jobs
        ),
        labor AS (
            SELECT SUM(p.burden_rate * TIMESTAMP_DIFF(ts.done_on, ts.dispatched_on, MINUTE) / 60.0) AS labor_cost
            ...
            WHERE ts._fivetran_deleted = FALSE AND ts.active = TRUE
              AND ts.dispatched_on IS NOT NULL AND ts.arrived_on IS NOT NULL AND ts.done_on IS NOT NULL
              AND t.name NOT LIKE '%Sales%' AND t.name NOT IN ('Propane Subs', 'Maybruck')
        )
        SELECT
            jt.gp_no_labor - COALESCE(l.labor_cost, 0) AS total_gp,
            jt.gp_no_labor, jt.total_revenue, jt.total_jobs,
    ...
    weekly = run_query(f"""
        WITH completed_jobs AS (
            SELECT j.id AS job_id, j.completed_on,
                COALESCE(fin.gross_profit, 0) AS gross_profit
            FROM `{PROJECT}.servicetitan.job` AS j
            LEFT JOIN (
                SELECT i.job_id, SUM(ii.total - COALESCE(ii.total_cost, 0)) AS gross_profit
                FROM `{PROJECT}.servicetitan.invoice` AS i
                JOIN `{PROJECT}.servicetitan.invoice_item` AS ii ON i.id = ii.invoice_id
                WHERE i._fivetran_deleted = FALSE AND i.active = TRUE
                  AND ii._fivetran_deleted = FALSE AND ii.active = TRUE
        ...
        weekly_rev AS (
            SELECT DATE_TRUNC(DATE(completed_on), WEEK(MONDAY)) AS week_start,
                SUM(gross_profit) AS gp_no_labor
            FROM completed_jobs GROUP BY week_start
        ),
        weekly_labor AS (
            SELECT DATE_TRUNC(DATE(j.completed_on), WEEK(MONDAY)) AS week_start,
        ...
        st.altair_chart(
            alt.Chart(weekly).mark_line(point=True).encode(
                x=alt.X("week_start:T", title="Week Starting",
                         axis=alt.Axis(format="%m/%d/%Y", labelAngle=-25)),
...
# ─────────────────────────────────────────────
    ...
    tech_data = run_query(f"""
        ...
        tech_job_time AS (
            SELECT ts.job_id, ts.technician_id, t.name AS technician_name,
        ...
        job_allocation AS (
            SELECT job_id, SUM(worked_hours) AS total_worked_hours FROM tech_job_time GROUP BY job_id
        ),
        tech_job_gp AS (
            SELECT tjt.technician_name, tjt.technician_id, tjt.job_id,
                CASE WHEN ja.total_worked_hours > 0
                    THEN j.gross_profit * tjt.worked_hours / ja.total_worked_hours
                    ELSE 0 END AS attributed_gp_no_labor,
                tjt.labor_cost, tjt.on_site_hours, tjt.drive_hours
        ...
        tech_rollup AS (
            SELECT technician_name, technician_id,
                COUNT(DISTINCT job_id) AS jobs,
                SUM(attributed_gp_no_labor - labor_cost) AS total_gp,
                SUM(on_site_hours) AS on_site_hours,
                SUM(drive_hours)   AS drive_hours
            FROM tech_job_gp GROUP BY technician_name, technician_id
        ),
        unassigned_jobs AS (
            SELECT 'Unassigned / Missing Job Time' AS technician_name,
                CAST(NULL AS INT64) AS technician_id,
                COUNT(*) AS jobs, SUM(j.gross_profit) AS total_gp,
                0.0 AS on_site_hours, 0.0 AS drive_hours
            FROM completed_jobs AS j
            LEFT JOIN job_allocation AS ja ON j.job_id = ja.job_id
            WHERE ja.job_id IS NULL OR COALESCE(ja.total_worked_hours, 0) = 0
        ),
        non_job AS (
            SELECT technician_id,
                SUM(SAFE_CAST(SPLIT(duration, ':')[OFFSET(0)] AS FLOAT64) +
                    SAFE_CAST(SPLIT(duration, ':')[OFFSET(1)] AS FLOAT64) / 60.0) AS non_job_hours
            FROM `{PROJECT}.servicetitan.non_job_appointment`
            WHERE active = TRUE AND _fivetran_deleted = FALSE
              AND start BETWEEN '{start_date}' AND '{end_date}'
            GROUP BY technician_id
        ),
        final_rollup AS (
            SELECT technician_name, technician_id, jobs, total_gp, on_site_hours, drive_hours
            FROM tech_rollup
            UNION ALL
            SELECT technician_name, technician_id, jobs, total_gp, on_site_hours, drive_hours
            FROM unassigned_jobs WHERE jobs > 0
        )
        SELECT fr.technician_name, fr.jobs, fr.total_gp,
            fr.on_site_hours, fr.drive_hours,
            ...
            COALESCE(SAFE_DIVIDE(fr.total_gp,
                fr.on_site_hours + fr.drive_hours + COALESCE(nj.non_job_hours, 0)), 0) AS gp_per_total_hr
    ...
    if not tech_data.empty:
        ...
        display = tech_data[["technician_name","jobs","total_gp","on_site_hours",
                              "drive_hours","non_job_hours","total_hours",
                              "gp_per_job_hr","gp_per_total_hr"]].copy()
        display.columns = ["Technician","Jobs","Gross Profit","On-Site Hrs",
                           "Drive Hrs","Non-Job Hrs","Total Hrs","GP / Job Hr","GP / Total Hr"]
        display.index = range(1, len(display) + 1)
        display.index.name = "Rank"
        for col in ["Gross Profit","GP / Job Hr","GP / Total Hr"]:
            display[col] = display[col].round(0).astype(int)
        for col in ["On-Site Hrs","Drive Hrs","Non-Job Hrs","Total Hrs"]:
...
# ─────────────────────────────────────────────
    ...
    calls_kpi = run_query(f"""
        ...
        WHERE _fivetran_deleted = FALSE AND lead_call_direction = 'Inbound'
          AND lead_call_created_on BETWEEN '{start_date}' AND '{end_date}'
    """)

    c1,c2,c3,c4,c5,c6 = st.columns(6)
    c1.metric("Inbound Calls", f"{int(calls_kpi['total_inbound'].iloc[0] or 0):,}")
    c2.metric("Booked",        f"{int(calls_kpi['booked'].iloc[0] or 0):,}")
    c3.metric("Unbooked",      f"{int(calls_kpi['unbooked'].iloc[0] or 0):,}")
    c4.metric("Excused",       f"{int(calls_kpi['excused'].iloc[0] or 0):,}")
    c5.metric("Abandoned",     f"{int(calls_kpi['abandoned'].iloc[0] or 0):,}")
    c6.metric("Booking Rate",  f"{float(calls_kpi['booking_rate'].iloc[0] or 0):.1%}")

    st.markdown("")
    st.subheader("Booking Rate Over Time")
    br_time = run_query(f"""
        SELECT DATE_TRUNC(DATE(lead_call_created_on), WEEK(MONDAY)) AS week_start,
            COUNT(*) AS inbound,
            COUNTIF(lead_call_call_type = 'Booked')   AS booked,
            COUNTIF(lead_call_call_type = 'Unbooked') AS unbooked,
            COUNTIF(lead_call_call_type = 'Excused')  AS excused,
    ...
    if not br_time.empty:
        ...
        st.altair_chart(
            alt.Chart(br_time).mark_line(point=True).encode(
                x=alt.X("week_start:T", title="Week Starting",
                         axis=alt.Axis(format="%m/%d/%Y", labelAngle=-25)),
                y=alt.Y("booking_rate:Q", title="Booking Rate",
    ...
    agents = run_query(f"""
        SELECT lead_call_agent_name AS agent, COUNT(*) AS total_calls,
            COUNTIF(lead_call_call_type = 'Booked')    AS booked,
            COUNTIF(lead_call_call_type = 'Unbooked')  AS unbooked,
            COUNTIF(lead_call_call_type = 'Excused')   AS excused,
            COUNTIF(lead_call_call_type = 'Abandoned') AS abandoned,
        ...
        WHERE _fivetran_deleted = FALSE AND lead_call_direction = 'Inbound'
          ...
          AND lead_call_created_on BETWEEN '{start_date}' AND '{end_date}'
        GROUP BY agent HAVING COUNT(*) >= 5 ORDER BY booking_rate DESC
    """)
    if not agents.empty:
    ...
    campaigns = run_query(f"""
        SELECT COALESCE(c.name, 'Unknown') AS campaign,
            COUNT(*) AS calls,
        ...
        WHERE cl._fivetran_deleted = FALSE AND cl.lead_call_direction = 'Inbound'
          AND cl.lead_call_created_on BETWEEN '{start_date}' AND '{end_date}'
        GROUP BY campaign HAVING COUNT(*) >= 3 ORDER BY calls DESC LIMIT 15
    """)
